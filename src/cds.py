#!/usr/bin/env python3
"""
trans.py - Process transcript CDS coverage data
"""
...
def process_files(file_a_path, file_b_path, file_c_path, output_path, up_threshold, down_threshold):
    ...
    with open(file_b_path, 'r') as f:
        ...
        for line in f:
            ...
            elif feature_type == 'CDS' and gene_type == 'protein_coding' and current_trans_id and current_trans_id in transcripts:
                chrom = parts[0]
                start = int(parts[2]) - 1  # 转换为0-based
                end = int(parts[3])
                transcripts[current_trans_id]['cds'].append((chrom, start, end))
    ...
    for trans_id in trans_info:
        ...
        # 新增检查：跳过没有CDS记录的转录本
        if not t_data['cds']:  # 检查CDS列表是否为空
            continue
        total_poor = total_other = total_full = total_bases = 0
        
        for (chrom, start, end) in t_data['cds']:
            key = (chrom, start, end)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Process transcript CDS coverage data',
    ...
    required.add_argument('--coverage', required=True, 
                         ...
                         help='CDS coverage input file of the sample')
    ...
    optional.add_argument('--up-gene', type=int, default=20,
                         metavar='<int>',
                         help='CDS coverage reached the standard threshold (default: 20)')
    optional.add_argument('--down-gene', type=int, default=30,
                         metavar='<int>',
                         help='CDS coverage falls below the standard threshold (default: 30)')
    ...
    # 构建输出文件路径
    output_file = f"{args.out}_trans_cds.tsv"
