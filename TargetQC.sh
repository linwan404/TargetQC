...
UPSTREAM=150
DOWNSTREAM=150
GENE="${SCRIPT_DIR}/data/target_transcript.tsv"
REFERENCE="${SCRIPT_DIR}/data/genes.gtf"
REF_BED="${SCRIPT_DIR}/data/HG001_GRCh38_1_22_v4.2.1_benchmark.bed"
REF_VCF="${SCRIPT_DIR}/data/HG001_GRCh38_1_22_v4.2.1_benchmark.vcf.gz"
REF_FASTA="${SCRIPT_DIR}/data/GCA_000001405.15_GRCh38_no_alt_analysis_set.fasta"
PATHOGENIC="${SCRIPT_DIR}/data/clinvar.txt"
...
usage() {
    ...
    echo "  --gene <gene_file>             Target genes. Such as protein-coding genes with clear genetic patterns in OMIM database (default: test_trans.tsv)"
    echo "  --reference <reference_file>   Gene annotation file (default: genes.gtf)"
    echo "  --pathogenic <pathogenic_file> Target variant sites (default: clinvar.txt)"
    ...
    echo "  --ref-bed <ref_bed>            Benchmark BED file (required for WGS when --sample-type is standard) (default: HG001_GRCh38_1_22_v4.2.1_benchmark.bed)"
    echo "  --ref-vcf <ref_vcf>            Benchmark VCF file (required when --sample-type is standard) (default: HG001_GRCh38_1_22_v4.2.1_benchmark.vcf.gz)"
    echo "  --ref-fasta <ref_fasta>        Human reference genome fasta file (required when --sample-type is standard) (default: GCA_000001405.15_GRCh38_no_alt_analysis_set.fasta)"
...
usage_variant_calling() {
    ...
    echo "  --capture <capture.bed>        Capture regions (.bed). Required for ES sequencing type"
    echo "  --ref-bed <ref_bed>            Benchmark BED file. Required for WGS sequencing type(default: HG001_GRCh38_1_22_v4.2.1_benchmark.bed)"
    echo "  --ref-vcf <ref_vcf>            Reference dataset VCF file (required when --sample-type is standard) (default: HG001_GRCh38_1_22_v4.2.1_benchmark.vcf.gz)"
    echo "  --ref-fasta <ref_fasta>        Human reference genome fasta file (default: GCA_000001405.15_GRCh38_no_alt_analysis_set.fasta)"
    echo "  --pathogenic <pathogenic_file> Target variant sites (default: clinvar.txt)"
    echo "  --out <outprefix>              Output prefix"
    echo ""
    echo "Additional general options:"
...
case $MODE in
    "exon")
#        if [[ -z "$BAM_FILE" || -z "$GENE" || -z "$REFERENCE" || -z "$OUT" ]]; then
#            echo "Error: exon mode requires --bam, --gene, --reference and --out parameters!"
        if [[ -z "$BAM_FILE" || -z "$OUT" ]]; then
            echo "Error: gene mode requires --bam and --out parameters!"
...
#            echo "Error: gene mode requires --bam, --gene, --reference and --out parameters!"
        ...
        ;;
    "variant-calling")
#        if [[ -z "$VCF_FILE" || -z "$GVCF_FILE" || -z "$REF_BED" || -z "$REF_VCF" || -z "$REF_FASTA" || -z "$PATHOGENIC" || -z "$OUT" ]]; then
#            echo "Error: variant-calling mode requires --vcf, --gvcf, --ref-bed, --ref-vcf, --ref-fasta, --pathogenic and --out parameters!"
        if [[ -z "$VCF_FILE" || -z "$GVCF_FILE" || -z "$OUT" ]]; then
            echo "Error: variant-calling mode requires --vcf, --gvcf and --out parameters!"
            echo "Use TargetQC.sh --mode variant-calling -h for detailed help information"
            exit 1
        fi
        ;;
    "variant-capture")
#        if [[ -z "$VCF_FILE" || -z "$PATHOGENIC" || -z "$OUT" ]]; then
#            echo "Error: variant-capture mode requires --vcf, --pathogenic and --out parameters!"
        if [[ -z "$VCF_FILE" || -z "$OUT" ]]; then
            echo "Error: variant-capture mode requires --vcf and --out parameters!"
            echo "Use TargetQC.sh --mode variant-capture -h for detailed help information"
            exit 1
        fi
        ;;
    "both")
#        if [[ -z "$SAMPLE_TYPE" || -z "$BAM_FILE" || -z "$VCF_FILE" || -z "$GENE" || -z "$REFERENCE" || -z "$PATHOGENIC" || -z "$OUT" ]]; then
#            echo "Error: full flow analysis mode requires --sample-type, --bam, --vcf, --gene, --reference, --pathogenic and --out parameters!"
        if [[ -z "$SAMPLE_TYPE" || -z "$BAM_FILE" || -z "$VCF_FILE" || -z "$OUT" ]]; then
            echo "Error: full flow analysis mode requires --sample-type, --bam, --vcf and --out parameters!"
            echo "Use TargetQC.sh -h for detailed help information"
            exit 1
        fi
#        if [[ "$SAMPLE_TYPE" == "standard" && (-z "$GVCF_FILE" || -z "$REF_BED" || -z "$REF_VCF" || -z "$REF_FASTA") ]]; then
#            echo "Error: standard sample type requires --gvcf, --ref-bed, --ref-vcf and --ref-fasta parameters!"
        if [[ "$SAMPLE_TYPE" == "standard" && -z "$GVCF_FILE" ]]; then
            echo "Error: standard sample type requires --gvcf parameters!"
            echo "Use TargetQC.sh -h for detailed help information"
            exit 1
        fi
        ;;
